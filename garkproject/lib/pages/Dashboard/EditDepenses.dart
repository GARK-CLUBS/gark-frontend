...
class EditDepenses extends StatefulWidget {
  const EditDepenses({Key? key, required this.type, required this.id})
      : super(key: key);
  final String type;
  final String id;
  @override
  State<EditDepenses> createState() => _EditDepensesState();
}
...
class _EditDepensesState extends State<EditDepenses> {
  ...
  Future<void> GetAllDepenses() async {
    try {
      ...
      var response = await http.get(
          Uri.parse("http://10.0.2.2:3000/api/depanses/" + widget.id),
          headers: headers);
      if (response.statusCode == 200) {
        var data = jsonDecode(response.body);
        print(data);
        namepersons.text = data["sousType"];
              ...
              child: ElevatedButton(
                onPressed: () async {
                  if (_keyForm.currentState!.validate()) {
                    ...
                    http
                        .put(Uri.http(_baseUrl, "/api/depanses/" + widget.id),

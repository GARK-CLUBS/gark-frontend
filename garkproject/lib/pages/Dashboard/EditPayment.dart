...
class EditPayment extends StatefulWidget {
  const EditPayment({Key? key, required this.id}) : super(key: key);
  final String id;
...
class _EditPaymentState extends State<EditPayment> {
  ...
  Future<void> GetAllPayment() async {
    try {
      ...
      var response = await http.get(
          Uri.parse("http://10.0.2.2:3000/api/payments/" + widget.id),
          headers: headers);
      if (response.statusCode == 200) {
        var data = jsonDecode(response.body);
        print(data);
        namepersons2.text = data["PayedFor"];
        _pack.text = data["pack"];
        _frais.text = data["fraisDossier"].toString();
        _amount.text = data["montant"].toString();
        dropdownAbonnement = data["type"];
      } else {
        return Future.error("server error");
      }
    } catch (e) {
      ...
      child: SingleChildScrollView(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          DropdownButtonFormField<String>(
            value: dropdownAbonnement,
            hint: Text("Type d'abonnement"),
            onChanged: (String? newValue) {
              setState(() {
                dropdownAbonnement = newValue;
              });
            },
            validator: (String? value) {
              if (value?.isEmpty ?? true) {
                return 'Please enter a valid type of Abonnement';
              }
            },
            items:
                itemsAbonnement.map<DropdownMenuItem<String>>((String value) {
                ...
                onPressed: () async {
                  if (_keyForm.currentState!.validate()) {
                    ...
                    Map<String, dynamic> userData = {
                      "PayedFor": namepersons2.text,
                      "montant": _amount.text,
                      "pack": _pack.text,
                      "type": dropdownAbonnement,
                      "fraisDossier": _frais.text,
                      "membre": membreData,
                      "club": ClubData,
                    };
                    ...
                    http
                        .put(Uri.http(_baseUrl, "/api/payments/" + widget.id),

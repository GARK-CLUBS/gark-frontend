...
class EditTache extends StatefulWidget {
  const EditTache({Key? key, required this.id}) : super(key: key);
  final String id;
  @override
  State<EditTache> createState() => _EditTacheState();
}
...
class _EditTacheState extends State<EditTache> {
  ...
  Future<bool> fetchDocss() async {
    ...
    http.Response response = await http.get(
        Uri.parse("http://10.0.2.2:3000/api/types/" + '${widget.id}'),
        headers: headers);

    if (response.statusCode == 200) {
      var data = json.decode(response.body);
      print(data);
      _nom.text = data["typechoix"];
      nameicon = data["nomIcon"];
// dont forget icon seelcted !!
